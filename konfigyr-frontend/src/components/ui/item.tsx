  ...
  'group/item flex flex-wrap items-center rounded-md border border-transparent text-sm transition-colors duration-100 outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 [a]:transition-colors [a]:hover:bg-accent/50',
  {
    variants: {
      variant: {
        ...
        list: 'rounded-none border-b-border last:border-transparent -mx-3',
        muted: 'bg-muted/50',
      },
      size: {
...
export function ItemTitle({ className, ...props }: ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('font-heading line-clamp-1 flex w-fit items-center gap-2 leading-snug font-medium underline-offset-4', className)}
      {...props}
    />
  );
}
