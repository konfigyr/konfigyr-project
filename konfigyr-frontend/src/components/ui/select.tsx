...
export function SelectLabel({ className, reset = false, onReset, children, ...props }: SelectPrimitive.GroupLabel.Props & {
  reset?: boolean
  onReset?: () => void
}) {
  return (
    <SelectPrimitive.GroupLabel
      ...
      className={cn(
        ...
        reset && 'flex items-center gap-2 justify-between',
        className,
      )}
      {...props}
    >
      ...
      {reset && (
        <SelectPrimitive.Icon
          data-slot="select-label-close"
          render={
            <XIcon className="size-4 cursor-pointer opacity-50 hover:opacity-100" />
          }
          onClick={onReset}
        />
      )}
...
export function SelectScrollDownButton({ className, ...props }: SelectPrimitive.ScrollDownArrow.Props) {
  return (
