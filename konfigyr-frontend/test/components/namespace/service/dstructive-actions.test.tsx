...
describe('components | namespace | service | <ServiceDestructiveActions/>', () => {
  ...
  beforeEach(() => {
    result = renderWithQueryClient((
      <ServiceDestructiveActions namespace={namespaces.konfigyr} service={services.konfigyrId} />
    ));
  });
  ...
  test('should open the delete confirmation modal', async () => {
    ...
    await waitFor(() => {
      expect(result.getByText(`Delete "${services.konfigyrId.name}" service`)).toBeInTheDocument();
    });
